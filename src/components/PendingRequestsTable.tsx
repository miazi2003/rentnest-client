...
interface PendingRequestsTableProps {
  rentals?: any; // Accept direct return value of rentalActions()
...
export function PendingRequestsTable({
  ...
  const displayRequests = React.useMemo(() => {
    return pendingItems.slice(0, 5).map((item: any, idx: number) => {
      const propTitle =
      ...
      const tenantName =
        typeof item.tenant === "string"
          ...
          ? item.tenantName
          : item.tenant?.name || item.user?.name || item.tenant?.email || item.user?.email || "Applicant Tenant";

      const tenantEmail =
        typeof item.tenantEmail === "string"
          ? item.tenantEmail
          : item.tenant?.email || item.user?.email || "";
